...
typedef NS_ENUM(NSUInteger, CAPSPageMenuScrollDirection) { CAPSPageMenuScrollDirectionLeft, CAPSPageMenuScrollDirectionRight, CAPSPageMenuScrollDirectionOther };
...
/// view scrollview 到第一张或者最后一张是否能滑动
...
/// 底部滑动条 宽度比(总宽度 - x%*2)
NSString *const CAPSPageMenuOptionSelectionPercentageMargin = @"selectionPercentageMargin";
/// 控制器的滚动视图是否可以滚动
NSString *const CAPSPageMenuOptionControllerScrollViewScrollEnabled = @"controllerScrollViewScrollEnabled";
/// menu 的frame 的Y
NSString *const CAPSPageMenuOptionControllerMenuOffsetY = @"controllerMenuOffsetY";
/// item 宽度根据文字定宽度时 底部滑动条向前偏移的量
NSString *const CAPSPageMenuOptionControllerMenuItemSelectionIndicatorX = @"menuItemSelectionIndicatorX";
/// item 宽度根据文字定宽度时 首尾item与item间距的差值，item间距 - 首尾item想要设置的间距
NSString *const CAPSPageMenuOptionControllerMenuItemHeadFootDifference = @"menuItemHeadFootDifference";
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options {
    ...
    if (options) {
        for (NSString *key in options) {
            ...
            } else if ([key isEqualToString:CAPSPageMenuOptionSelectionPercentageMargin]) {
                _selectionPercentageMargin = [options[key] floatValue];
            } else if ([key isEqualToString:CAPSPageMenuOptionControllerScrollViewScrollEnabled]) {
                _controllerScrollViewScrollEnabled = [options[key] boolValue];
            } else if ([key isEqualToString:CAPSPageMenuOptionControllerMenuOffsetY]) {
                _menuOffsetY = [options[key] integerValue];
            } else if ([key isEqualToString:CAPSPageMenuOptionControllerMenuItemSelectionIndicatorX]) {
                _menuItemSelectionIndicatorX = ([options[key] floatValue] >= 0.0) ? [options[key] floatValue] : 0.0;
            } else if ([key isEqualToString:CAPSPageMenuOptionControllerMenuItemHeadFootDifference]) {
                _menuItemHeadFootDifference = ([options[key] floatValue] >= 0.0) ? [options[key] floatValue]: 0.0;
            }
        }

        CGFloat mainScreenWidth = [[UIScreen mainScreen] bounds].size.width;
        if (_menuItemWidth * [viewControllers count] > mainScreenWidth) {
...
- (void)initValues {
    ...
    _selectionPercentageMargin = 0.0;
    _controllerScrollViewScrollEnabled = YES;
    ...
    _menuItemSelectionIndicatorX = 0.0;
    _menuItemHeadFootDifference = 0.0;
}

- (void)setUpUserInterface {
    NSDictionary *viewsDictionary = @{ @"menuScrollView": _menuScrollView, @"controllerScrollView": _controllerScrollView };

    _controllerScrollView.pagingEnabled = YES;
    _controllerScrollView.scrollEnabled = _controllerScrollViewScrollEnabled;
    ...
    //    _controllerScrollView.layer.borderWidth = 2.0;
    //    _controllerScrollView.layer.borderColor = [[UIColor darkGrayColor] CGColor];
    ...
    _menuScrollView.frame = CGRectMake(0.0, _menuOffsetY, self.view.frame.size.width, _menuHeight);
...
- (void)configureUserInterface {
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            menuItemFrame = CGRectMake(_totalMenuItemWidthIfDifferentWidths + _menuMargin + (_menuMargin * index) - _menuItemHeadFootDifference, 0.0, _menuItemWidth, _menuHeight);
    ...
    if (_menuItemWidthBasedOnTitleTextWidth) {
        _menuScrollView.contentSize = CGSizeMake((_totalMenuItemWidthIfDifferentWidths + _menuMargin) + (CGFloat)_controllerArray.count * _menuMargin - 2 * _menuItemHeadFootDifference, _menuHeight);
    }
    ...
    if (_useMenuLikeSegmentedControl) {
        selectionIndicatorFrame = CGRectMake(0.0 + _menuItemWidth * _selectionPercentageMargin, _menuHeight - _selectionIndicatorHeight, self.view.frame.size.width / (CGFloat)_controllerArray.count - _menuItemWidth * _selectionPercentageMargin * 2, _selectionIndicatorHeight);
    } else if (_menuItemWidthBasedOnTitleTextWidth) {
        selectionIndicatorFrame = CGRectMake(_menuMargin - _menuItemSelectionIndicatorX - _menuItemHeadFootDifference, _menuHeight - _selectionIndicatorHeight, [_mutableMenuItemWidths[0] floatValue] + 2 * _menuItemSelectionIndicatorX, _selectionIndicatorHeight);
    } else {
        if (_centerMenuItems) {
            ...
            if (scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= ((CGFloat)(_controllerArray.count - 1) * self.view.frame.size.width)) {
                ...
                if ((_currentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation)) || (!_currentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation))) {
                    ...
                    if (_menuScrollView.contentSize.width > self.view.frame.size.width) {
                        ...
                        /*
                        CGFloat desiredX = _menuItemWidth * _currentPageIndex - (_menuScrollView.bounds.size.width / 2);
                        if (desiredX < 0)
                            desiredX = 0;

                        if (desiredX > _menuItemWidth * [_controllerArray count] - _menuScrollView.bounds.size.width)
                            desiredX = _menuItemWidth * [_controllerArray count] - _menuScrollView.bounds.size.width;
                        [_menuScrollView setContentOffset:CGPointMake(desiredX, 0) animated:NO];
                         */
                    }
        ...
        [UIView animateWithDuration:0.15
                         animations:^{
                             ...
                             if (self.useMenuLikeSegmentedControl) {
                                 selectionIndicatorX = (CGFloat)pageIndex * (self.view.frame.size.width / (CGFloat)self.controllerArray.count) + _menuItemWidth * _selectionPercentageMargin;
                                 selectionIndicatorWidth = self.view.frame.size.width / (CGFloat)self.controllerArray.count - _menuItemWidth * _selectionPercentageMargin * 2;
                             } else if (self.menuItemWidthBasedOnTitleTextWidth) {
                                 selectionIndicatorWidth = [self.menuItemWidths[pageIndex] floatValue] + 2 * _menuItemSelectionIndicatorX;
                                 selectionIndicatorX += (self.menuMargin - _menuItemSelectionIndicatorX - _menuItemHeadFootDifference);
...
- (void)handleMenuItemTap:(UITapGestureRecognizer *)gestureRecognizer {
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            CGFloat menuItemRightBound = (_menuMargin- _menuItemHeadFootDifference) + [_mutableMenuItemWidths[0] floatValue] + (_menuMargin / 2);
...
- (void)addPageAtIndex:(NSInteger)index {
    ...
    newVC.view.frame = CGRectMake(_controllerScrollView.frame.size.width * (CGFloat)index, _menuHeight, _controllerScrollView.frame.size.width, _controllerScrollView.frame.size.height - _menuHeight);
    ...
    /*
    if (index == 0) {
        newVC.view.layer.borderWidth = 1.0;
        newVC.view.layer.borderColor = [[UIColor greenColor] CGColor];
    }
     */
}
...
- (void)viewDidLayoutSubviews {
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            _selectionIndicatorView.frame = CGRectMake(selectionIndicatorX + _menuItemWidth * _selectionPercentageMargin, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth - _menuItemWidth * _selectionPercentageMargin * 2, self.selectionIndicatorView.frame.size.height);
...
- (void)moveToPage:(NSInteger)index {
    [self moveToPage:index animate:YES completion:nil];
}
